...
상대 AI모드 켜기/끄기 Z (AI는 항상 위쪽임)
AI 훈수받기 X (아랫쪽 1회 AI)
판 좌우　반전　［
판　상하　반전　］　반전시에　기존　저장된　수순을　바꿔서　저장하는　기능　자동　탑재
상하　반전이　있을　때는　ai가 위쪽을 가져감
단축키나 편집기가 열려있을 때 esc는 열려있는 편집기를 저장하지 않고 닫아줌
편집기에서 코멘트 나타내는 시간을 보여줌, 0은 무제한(클릭하면 숨김)
