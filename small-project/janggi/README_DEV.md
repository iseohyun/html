...
- **선택 취소 및 상차림 복귀 (Escape)**: 
  - 기물이 선택된 상태에서 ESC 키를 누르면 **선택 취소(해제)** 처리됩니다.
  - 선택이 해제된 상태(혹은 커서 모드 해제 상태)에서 ESC 키를 한 번 더 누르면 **설정창(상차림 설정)으로 복귀**합니다.
- **키보드 커서락 모드 (Cursor Lock Mode, CapsLock)**:
  - 설정 카테고리 4(설정)에서 온오프 하거나, 대국 중 **CapsLock** 키를 눌러 활성화 상태를 실시간 토글할 수 있습니다. (토글 시 토스트 알림창 고지)
  - **작동 원리**:
    - **선택된 기물이 없을 때**: 방향키(특히 좌우 화살표) 입력 시 판 위에서 자유롭게 이동하는 대신, **움직일 수 있는 아군 기물들 사이에서만 포커스가 순환 이동**합니다.
    - **기물이 선택되어 있을 때**: 방향키 입력 시 **해당 기물의 가용 이동 후보지(동작 가능 영역) 내에서만 포커스가 순환 이동**합니다.
    - 이를 통해 불필요한 빈칸 탐색을 줄이고 빠르고 정확하게 행마를 수행할 수 있는 극대화된 접근성을 보장합니다.
