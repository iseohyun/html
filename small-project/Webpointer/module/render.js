...
(function() {
  ...
  var WebpointerRender = {
    ...
    // Render Ribbon Bar Controls dynamically based on current active tab
    renderRibbon: function() {
      ...
      if (cfg.currentTab === 'insert') {
        ...
        ribbonBar.innerHTML = 
          self.buildCategoryHtml('insert_shapes', '도형 삽입', self.build3RowGridHtml(shapeTools)) +
          self.buildCategoryHtml('insert_layer', '레이어 순서', self.build3RowGridHtml(layerTools)) +
          self.buildCategoryHtml('insert_group', '그룹화', self.build3RowGridHtml(groupTools)) +
          self.buildCategoryHtml('insert_align', '정렬 및 간격', self.build3RowGridHtml(alignTools)) +
          self.buildCategoryHtml('insert_transform', '회전 및 대칭', self.build3RowGridHtml(transformTools));
        ...
        ribbonBar.innerHTML = 
          self.buildCategoryHtml('view_grid', '격자 및 스냅/선택/기본크기 설정', gridSettingsHtml) +
          self.buildCategoryHtml('view_canvas', '캔버스 화면 설정', canvasSettingsHtml);
        ...
        ribbonBar.innerHTML =
          self.buildCategoryHtml('style_color', '색', colorContent) +
          self.buildCategoryHtml('style_line', '선', lineContent) +
          self.buildCategoryHtml('style_lineEnds', '선 끝', lineEndsContent) +
          self.buildCategoryHtml('style_capJoin', '마감', capJoinContent);
        ...
        var textContent =
          ...
          '</div>';
        ribbonBar.innerHTML = self.buildCategoryHtml('text_font', '글 서식', textContent);
        ...
        var animContent =
          ...
          '</div>';
        ribbonBar.innerHTML = self.buildCategoryHtml('anim_preview', '애니메이션 미리보기', animContent);
      }
    },
