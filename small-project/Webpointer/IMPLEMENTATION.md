...
### 2.1 객체 바디 드래그 이동 (Object Drag-Move System)
...
### 2.5 베지어 곡선 (Bezier Curves: 2차 & 3차)
* **2차 베지어 (Quadratic Bezier, 1 핸들)**:
  * **시작점 핸들 (`start`)**: $(x_1, y_1)$ 위치 (하얀색 `#ffffff` / 검은 테두리 `#000000`)
  * **끝점 핸들 (`end`)**: $(x_2, y_2)$ 위치 (하얀색 `#ffffff` / 검은 테두리 `#000000`)
  * **중간 제어점 핸들 (`bez2_ctrl`)**: $(cx, cy)$ 위치 (**노란색 `#facc15` / 검은 테두리 `#000000`**)
* **3차 베지어 (Cubic Bezier, 2 핸들)**:
  * **시작점 핸들 (`start`)**: $(x_1, y_1)$ 위치 (하얀색 `#ffffff` / 검은 테두리 `#000000`)
  * **끝점 핸들 (`end`)**: $(x_2, y_2)$ 위치 (하얀색 `#ffffff` / 검은 테두리 `#000000`)
  * **제어점 1 핸들 (`bez3_ctrl1`)**: $(c1x, c1y)$ 위치 (**노란색 `#facc15` / 검은 테두리 `#000000`**)
  * **제어점 2 핸들 (`bez3_ctrl2`)**: $(c2x, c2y)$ 위치 (**노란색 `#facc15` / 검은 테두리 `#000000`**)

---

### 2.6 둥근 사각형 (Rounded Rectangle)
* **조종점 핸들 구성 (3종)**:
  1. **좌상단 핸들 (`top_left`)**: $(x, y)$ 위치 (하얀색 `#ffffff` / 검은 테두리 `#000000`) $\rightarrow$ 사각형 위치 및 상단/좌측 크기 조절
  2. **우하단 핸들 (`bottom_right`)**: $(x + w, y + h)$ 위치 (하얀색 `#ffffff` / 검은 테두리 `#000000`) $\rightarrow$ 사각형 하단/우측 크기 조절
  3. **굴곡각 핸들 (`corner_rx`)**:
     * **배치 위치**: **좌상단 모서리의 굴곡이 끝나는 상단 변 위치 `(x + rx, y)`**
     * **외형**: **노란색 배경 (`#facc15`), 검은색 테두리 (`#000000`, 1.5px)**
     * **동작**: 상단 변을 따라 드래그 시 모서리 굴곡 반지름 `rx`를 $0 \le rx \le \frac{\text{width}}{2}$ 범위 내에서 실시간 조절
