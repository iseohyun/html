...
### 2.2 연속 다중 클릭 베지어 곡선 입력 방식 (Continuous Multi-Click Bezier Input System)
* **최초 클릭 (First Click)**: 클릭한 위치 $(x_1, y_1)$가 베지어 곡선의 **시작점**이 됩니다.
* **실시간 이동 및 다음 좌표 준비 (Real-time Preview & Next Segment Readiness)**:
  * 마우스를 이동하면 시작점 $(x_1, y_1)$부터 **현재 마우스 커서 위치 $(x_2, y_2)$까지의 베지어 곡선 모습을 실시간 렌더링**합니다.
  * 다음 클릭을 진행하면 곡선 생성이 끝나지 않고, 이전 끝점 좌표가 새로운 시작점이 되며 **계속해서 다음 베지어 세그먼트 좌표를 입력받을 준비**를 합니다.
* **제어점 연산 규격**:
  * **1차 베지어 (bez2 / Q-Bezier)**: 제어점은 시작점의 X값과 현재 마우스 끝점의 Y값 $(x_1, y_2)$으로 지정
  * **2차 베지어 (bez3 / C-Bezier)**: 가상 중심점 $V = (x_1, y_2)$ 기준, 제어점 1은 $(x_1, \frac{y_1+y_2}{2})$, 제어점 2는 $(\frac{x_1+x_2}{2}, y_2)$ 지정
* **입력 종료 (Finish / Esc)**:
  * `Esc` 키를 누르면 연속 베지어 작성이 최종 완료되며 선택 도구(`select`)로 전환됩니다.
