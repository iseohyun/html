...
## [v1.5.0] - 2026-08-11
- **색상 및 레이아웃 출처 추적(Source Tracking) 전역 로깅 & 예외 안전(Fail-Safe) 히스토리 시스템 구축**:
  - `window.logColorChange` 전역 방어적 로거 시스템을 구축하여 2D 모달 피커, Native 컬러피커, 테마 드롭다운, 스포이드, Undo/Redo 등 색상이 변경되는 모든 위치의 발원지 태그(`[출처: ...]`) 정밀 로깅.
  - 2D 팔레트 모달 드래그 중의 60fps 미리보기 렌더링 유지 및 선택 확인 버튼(`btnPickerConfirm`) 클릭 시점 1회 정밀 최종 확정 로그 발행.
  - 핸들러 전역 `try-catch-finally` 감싸기로 스크립트 중단(사망) 원천 방지 및 메시지 출력 최우선 보장.
  - `applySettingsToUI` 네임스페이스 익스포트 복구로 Undo/Redo 시 이전 원본 색상 및 레이아웃 수치 100% 동기화 원복 완치.
  - Undo 실행 상태에서 새 작업 수행 시 미래 스택(Redo) 자동 삭제 및 대체(Branching) 구현.
  - 모든 레이아웃/일반 설정의 변경 내역을 `"변경내용","변경전 값","변경후 값"` 1:1 표준 규격으로 히스토리 카드 및 콘솔에 완벽 기록.
